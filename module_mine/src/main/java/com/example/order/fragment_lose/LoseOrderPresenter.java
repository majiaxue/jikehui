...
//        WaitDialog.show((AppCompatActivity)mContext,null);
    ...
    private void tbOrder() {
        Map map = MapUtil.getInstance().addParms("status", 2).addParms("type", 0).addParms("current", "1").addParms("pageSize", "1000").build();
    ...
    private void jdOrder() {
        Map map = MapUtil.getInstance().addParms("status", 2).addParms("type", 1).addParms("current", "1").addParms("pageSize", "1000").build();
        Observable observable = RetrofitUtil.getInstance().getApi(CommonResource.BASEURL_4001).getHead(CommonResource.QUERY_PDD_ORDER, map, SPUtil.getToken());
        RetrofitUtil.getInstance().toSubscribe(observable, new OnMyCallBack(new OnDataListener() {
            @Override
            public void onSuccess(String result, String msg) {
    ...
    private void pddOrder() {
        Map map = MapUtil.getInstance().addParms("status", 2).addParms("type", 2).addParms("current", "1").addParms("pageSize", "1000").build();
        Observable observable = RetrofitUtil.getInstance().getApi(CommonResource.BASEURL_4001).getHead(CommonResource.QUERY_PDD_ORDER, map, SPUtil.getToken());
        RetrofitUtil.getInstance().toSubscribe(observable, new OnMyCallBack(new OnDataListener() {
            @Override
            public void onSuccess(String result, String msg) {
